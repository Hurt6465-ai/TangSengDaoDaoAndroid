...
final class PartnerPendingMessageGateway {
    ...
    boolean handle(WKMsg wkMsg) {
        ...
        if (state.messageCount >= state.maxMessageCount) {
            toast("对方回复前最多发送3条消息");
            return true;
        }
    ...
    private void handleFailure(WKMsg wkMsg, String clientMsgNo, int retry, int code, String msg) {
        ...
        boolean retryable = retry < 2 && (code <= 0 || code >= 500 || value.contains("确认中") || value.contains("重试") || value.contains("网络") || value.contains("超时"));
        if (retryable) {
