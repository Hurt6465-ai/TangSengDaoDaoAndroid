...
public class PartnerListUser {
    ...
    public int is_new;
    // 服务端版本不统一，保留多个常见注册时间字段；Gson 会把数字原始值转换成字符串。
    ...
    public String registrationTimeRaw() {
        if (!TextUtils.isEmpty(created_at)) return created_at;
        if (!TextUtils.isEmpty(joined_at)) return joined_at;
        if (!TextUtils.isEmpty(registered_at)) return registered_at;
        return join_time == null ? "" : join_time;
    }
