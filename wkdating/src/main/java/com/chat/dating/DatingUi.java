...
public final class DatingUi {
    ...
    public static void bindCircularFlag(ImageView flagView, String countryCode) {
        if (flagView == null) return;
        String code = countryCode == null ? "" : countryCode.trim().toLowerCase(Locale.US);
        ...
        int resId = flagView.getResources().getIdentifier(
                "ic_flag_" + code,
                "drawable",
                flagView.getContext().getPackageName());
        if (resId == 0) {
            flagView.setImageDrawable(null);
            flagView.setVisibility(View.GONE);
            return;
