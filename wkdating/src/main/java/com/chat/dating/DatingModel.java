...
public class DatingModel extends WKBaseModel {
    ...
    public void updateSharedUser(JSONObject body, final Callback<Object> callback) {
        JSONObject safeBody = body == null ? new JSONObject() : body;
        request(createService(DatingService.class).updateSharedUser(safeBody), new IRequestResultListener<CommonResponse>() {
            @Override
            public void onSuccess(CommonResponse result) {
                int code = result == null ? HttpResponseCode.error : result.status;
                String msg = result == null ? "" : result.msg;
                if (callback != null) callback.onResult(code, msg, result);
            }
