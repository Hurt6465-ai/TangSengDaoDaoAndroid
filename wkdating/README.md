...
1. `dating_favorites` 收藏表、收藏列表、取消收藏接口。
2. `POST /v1/dating/swipes/undo`，每天免费撤回 3 次，只允许撤回尚未形成匹配的最后动作。
3. 服务端额度：男喜欢 40、女喜欢 60；收藏额度按配置；跳过只做频率限制。
4. `who-liked-me` 接口与会员权限，免费用户只返回数量和模糊信息。
5. 推荐接口服务端硬过滤双方异国恋意愿、30 天活跃度、严重举报和照片数量。
6. 管理端：举报列表、删除照片、下架交友资料、封禁/恢复交友权限、查看多次举报用户。
7. 推荐 session 曝光去重和真正 cursor 分页。
