 ...
 * - Use a dedicated WK_RTC_SIGNAL content type instead of WK_TEXT, so SDP/ICE cannot render
 *   as normal chat text even when a packet is briefly persisted.
 * - header.noPersist = true for non-INVITE packets; INVITE may sync briefly so background devices can ring.
...
public class RtcWukongSignalTransport implements RtcSignalTransport {
    ...
    public void sendSignal(String peerUid, String payload) throws Exception {
        ...
        RtcSignal signal = null;
        try { signal = RtcSignal.fromTransportText(payload); } catch (Exception ignored) {}
        String myUid = RtcSignalManager.get().myUid();
        if (signal != null) {
            if (!TextUtils.isEmpty(myUid) && TextUtils.equals(signal.toUid, myUid)) return;
            if (!TextUtils.isEmpty(myUid) && TextUtils.equals(peerUid, myUid)) return;
        }
        ...
        boolean reliableInvite = signal != null && RtcSignal.INVITE.equals(signal.type);
        applyRealtimeSignalOptions(options, !reliableInvite);
        ...
        markByReflection(content, !reliableInvite);
        WKIM.getInstance().getMsgManager().sendWithOptions(content, channel, options);
    }

    private void applyRealtimeSignalOptions(WKSendOptions options, boolean noPersist) {
        ...
        try {
            if (options.header != null) {
                // Non-INVITE packets are online-only. INVITE may be synced briefly so background devices can ring.
                options.header.noPersist = noPersist;
        ...
        markByReflection(options, noPersist);
        markByReflection(getFieldValue(options, "header"), noPersist);
        markByReflection(getFieldValue(options, "setting"), noPersist);
    }

    private void markByReflection(Object object) {
        markByReflection(object, true);
    }
    ...
    private void markByReflection(Object object, boolean noPersist) {
        ...
        setFieldValue(object, "persist", !noPersist);
        setFieldValue(object, "isPersist", !noPersist);
        setFieldValue(object, "is_persist", !noPersist);
